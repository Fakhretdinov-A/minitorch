...
def test_sigmoid(a: float) -> None:
    ...
    * It is strictly increasing.
    ...
    # assert sigmoid(a - EPS) < sigmoid(a) < sigmoid(a + EPS)
...
def test_symmetric() -> None:
    ...
    assert mul(-1, 2) == mul(2, -1)
    assert mul(0, 2) == mul(2, 0)
    assert mul(-1, -2) == mul(-2, -1)
    assert_close(mul(1.5, 2.7), mul(2.7, 1.5))
...
def test_distribute() -> None:
    ...
    """
    assert mul(3, add(1, 2)) == add(mul(3, 1), mul(3, 2))
    assert mul(-3, add(1, 2)) == add(mul(-3, 1), mul(-3, 2))
    assert mul(3, add(1, -2)) == add(mul(3, 1), mul(3, -2))
    assert_close(mul(3.1, add(1.3, -2.8)), add(mul(3.1, 1.3), mul(3.1, -2.8)))
...
def test_other() -> None:
    ...
    """
    x = 3
    assert eq(id(x), inv(inv(x)))
    assert eq(max(1, 2), 2)
    assert eq(add(x, neg(x)), 0)
